    ...
    def post(self):
        ...
        if action == USER_ACTION_REGISTER:
            ...
            data = {
                ...
                'msg': 'create cinema user success',
                'data': cinema_user
            }

            return marshal(data, single_cinema_user_fields)  # 以模版方式返回数据
        elif action == USER_ACTION_LOGIN:
            args_login = parse_login.parse_args()
            username = args_login.get('username')
            phone = args_login.get('phone')
            user = get_cinema_user(username) or get_cinema_user(phone)  # 根据用户名和手机号获取用户对象
            if not user:
                abort(400, msg="用户不存在")
            ...
            token = generate_cinema_user_token()  # 生成token
